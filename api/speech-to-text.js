...
// limit, is shared too. Giving this endpoint its own separate
// allowance on top of translate.js's would let the two together
// overrun what Sunbird actually allows - so this reads the SAME
// SUNBIRD_LIMIT_PER_MINUTE env var as translate.js and text-to-speech.js,
...
// No timeout previously existed here either - see the note in
// translate.js. A stalled Sunbird transcription call would otherwise
// hang for minutes instead of failing fast.
//
// IMPORTANT - this budget must stay comfortably under the maxDuration
// set for this function in vercel.json (currently 60s). Worst case
// here: MAX_QUEUE_WAIT_MS (8s) + 2 attempts x 20s + ~0.5s backoff =
// ~48.5s, leaving over 10s of margin under the 60s ceiling.
const SUNBIRD_TIMEOUT_MS = 20000;
...
// and try again automatically - up to SUNBIRD_MAX_ATTEMPTS times - instead
// of immediately failing. Also retries on a raw network failure or a stall
// past SUNBIRD_TIMEOUT_MS (a dropped connection or hung response), which a
// plain fetch() rejection would otherwise skip past or hang on forever.
async function callSunbirdWithRetry(url, options, maxAttempts = SUNBIRD_MAX_ATTEMPTS) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      const response = await fetchWithTimeout(url, options, SUNBIRD_TIMEOUT_MS);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!sunbirdResponse.ok) {
      ...
      // See the matching note in translate.js: log the full raw response
      // server-side, but never forward a raw HTML error page into the
      // on-screen message.
      console.error("Sunbird transcription error (status " + sunbirdResponse.status + "):", errorText);
