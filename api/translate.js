...
// Set via the SUNBIRD_LIMIT_PER_MINUTE environment variable, so raising
// or lowering this only ever needs a Vercel settings change - never a
// code edit or redeploy request. Set it a little UNDER whatever Sunbird
// actually gives your account (their dashboard/contract, not something
// this app can query), so this queue's own retries always have a small
// safety margin left over rather than racing right up against Sunbird's
// real wall. Falls back to 45 (Sunbird's original "Standard" free tier)
// if the env var isn't set.
const SUNBIRD_LIMIT_PER_MINUTE = parseInt(process.env.SUNBIRD_LIMIT_PER_MINUTE, 10) || 45;
...
// No timeout previously existed on the fetch() calls below - if Sunbird
// ever stalls instead of returning a fast error, the request would hang
// until Vercel's own platform-level function limit eventually killed
// it, which shows up to the person as "Translating......" for minutes
// with no error. This wraps every Sunbird call with a hard deadline, so
// a stall fails fast and clearly instead.
//
// IMPORTANT - this budget must stay comfortably under the maxDuration
// set for this function in vercel.json (currently 60s), or Vercel's own
// platform timeout kills the function first with a generic 504, before
// our own friendlier timeout/error message ever gets a chance to run.
// Worst case here: MAX_QUEUE_WAIT_MS (8s) + 2 attempts x 18s + ~0.5s
// backoff = ~44.5s, leaving over 15s of margin under the 60s ceiling.
const SUNBIRD_TIMEOUT_MS = 18000;
const SUNBIRD_MAX_ATTEMPTS = 2;
...
// hiccup (503), waits a moment and tries again automatically - up to
// SUNBIRD_MAX_ATTEMPTS times - instead of immediately failing. A stall
// past SUNBIRD_TIMEOUT_MS is treated the same as a retryable hiccup,
// rather than being left to hang.
async function callSunbirdWithRetry(url, options, maxAttempts = SUNBIRD_MAX_ATTEMPTS) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    let response;
    try {
      response = await fetchWithTimeout(url, options, SUNBIRD_TIMEOUT_MS);
    } catch (err) {
      if (attempt === maxAttempts) {
        throw new Error(err.name === "AbortError" ? "SUNBIRD_TIMEOUT" : err.message);
      }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!sunbirdResponse.ok) {
      ...
      // Log the FULL raw response server-side (visible in Vercel's
      // function logs) no matter what it looks like - that's where real
      // debugging happens. What we show the PERSON must stay short and
      // readable: if Sunbird (or a gateway/load balancer in front of it)
      // returned a full HTML error page instead of JSON - which happens
      // on some 5xx failures - dumping that raw HTML into the on-screen
      // message is unreadable and was showing up as a wall of markup
      // that then vanished before anyone could read it anyway.
      console.error("Sunbird translate error (status " + sunbirdResponse.status + "):", errorText);
      ...
      const looksLikeHtml = errorText.trim().startsWith("<");
      return res.status(sunbirdResponse.status).json({
        error: looksLikeHtml
          ? "Sunbird API error (status " + sunbirdResponse.status + ") - please try again in a moment."
          : "Sunbird API error: " + errorText
      });
    }
    ...
    if (err.message === "SUNBIRD_TIMEOUT") {
      return res.status(504).json({ error: "Sunbird isn't responding right now - please try again in a moment." });
    }
