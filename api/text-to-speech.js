...
// shared too - so this reads the SAME SUNBIRD_LIMIT_PER_MINUTE env var,
...
export default async function handler(req, res) {
  ...
  // No timeout previously existed on either Sunbird call below - see the
  // note in translate.js. A stalled Sunbird response would otherwise
  // hang for minutes instead of failing fast.
  // IMPORTANT - both timeouts in this file (and the two queue waits
  // around them) must stay comfortably under the maxDuration set for
  // this function in vercel.json (currently 60s). Worst case: queue
  // (8s) + voice lookup (10s) + queue (8s) + speech generation (20s) =
  // 46s, leaving ~14s of margin under the 60s ceiling.
  ...
  try {
    ...
    const timeout2 = setTimeout(() => controller2.abort(), 20000); // speech generation genuinely takes longer than a voice lookup

    let speechResponse;
    try {
      speechResponse = await fetch("https://api.sunbird.ai/tasks/audio/speech", {
        method: "POST",
        headers: {
          ...authHeader,
    ...
    if (!speechResponse.ok) {
      ...
      // on-screen message.
      console.error("Sunbird TTS error (status " + speechResponse.status + "):", errorText);
      const looksLikeHtml = errorText.trim().startsWith("<");
      return res.status(speechResponse.status).json({
        error: looksLikeHtml
          ? "Sunbird TTS error (status " + speechResponse.status + ") - please try again in a moment."
          : "Sunbird TTS error: " + errorText
      });
    }
