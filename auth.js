...
// Two accepted shapes for the phone field, decided by whether it starts
// with a "+":
//   - Local:         exactly 10 digits, e.g. "0771234567"
//   - Country code:  "+" then up to 15 digits, e.g. "+256771234567"
// This strips anything that isn't 0-9 or a leading "+" as someone types
// or pastes, then caps the digit count at 10 (local) or 15 (with a country
// code) - so the 11th digit of a local-format number can never land in
// the field, while a real international number still fits.
authPhone.addEventListener("input", () => {
  const hasPlus = authPhone.value.trim().startsWith("+");
  const digitsOnly = authPhone.value.replace(/\D/g, "");
  const maxDigits = hasPlus ? 15 : 10;
  authPhone.value = (hasPlus ? "+" : "") + digitsOnly.slice(0, maxDigits);
});
...
// Accepts either a local 10-digit number (e.g. "0771234567") or the
// normal way of writing it with a country code (e.g. "+256771234567").
// The input listener above already strips anything else as someone
// types or pastes, so in practice this mostly guards against submitting
// too few digits either way, not too many.
...
registerBtn.addEventListener("click", async () => {
  ...
  if (!isValidPhone(phone)) return showAuthError("Enter a valid phone number: 10 digits (e.g. 0771234567) or with a country code (e.g. +256771234567).");
...
loginBtn.addEventListener("click", async () => {
  ...
  // Remember the session id the server just handed us - every request
  // to a protected feature (translate, voice, subscribe...) sends this
  // back as the X-Session-Id header, so the server can tell this tab
  // apart from any other device that might log into the same account
  // later. See handleSessionConflict() below for what happens then.
