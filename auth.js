...
// MUST be read before supabase.createClient() below. The client's own
// URL-detection step (detectSessionInUrl, on by default) consumes and
// clears any #type=recovery... hash almost immediately as part of
// creating the client - so any later code that re-reads
// window.location.hash to check "was this a recovery link?" will find
// it already empty and get the wrong answer. Capturing it into a plain
// variable first, before the client has a chance to touch it, is what
// makes that check reliable everywhere else in this file.
...
// above handles instead). Uses the isPasswordRecoveryLink flag captured
// at the top of this file, NOT a fresh read of window.location.hash -
// by this point Supabase's own URL-detection has already cleared that
// hash, so re-reading it here would always say "not a recovery link"
// even when it was one, and send the person straight to the app instead
// of the new-password screen (and then straight back out again, since
// that tab has no active_sessions row yet).
(async () => {
  const { data } = await supabaseClient.auth.getSession();
  if (isPasswordRecoveryLink) {
    return; // the onAuthStateChange("PASSWORD_RECOVERY") listener owns this case
  }
  if (data.session) {
    showApp();
  } else {
