...
// English and Swahili use the browser's own free, instant speech
// recognition. Every other language records real audio and sends it to
// Sunbird's server for transcription, which needs an internet connection
// and takes a few seconds. Let people know that up front, instead of
// leaving them wondering why it behaves differently.
function updateVoiceNote() {
  const isInstantLang = srcLang.value === "eng" || srcLang.value === "swa";
  if (isInstantLang) {
    voiceNote.style.display = "none";
  } else {
    voiceNote.textContent = "Voice input for this language uses your internet connection and may take a few seconds.";
...
srcLang.addEventListener("change", updateVoiceNote);

// Show the right note as soon as the page loads, based on whatever
...
photoBtn.addEventListener("click", () => {
  // Tesseract (our OCR library) only reliably reads English text. If the
  // selected source language is anything else, warn before opening the
  // camera - otherwise it silently misreads the local-language text as
  // English letters, and that garbled text still gets "translated"
  // without any error, giving a confidently wrong result.
  if (srcLang.value !== "eng") {
    showMessage("Photo scanning currently reads English text only - results may be inaccurate for other languages.", 4000);
  }
  photoInput.click();
});
