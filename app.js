...
// Keeps track of the most recent translation, so the flag button
// knows exactly what to report.
...
translateBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    // Remember what was translated, so the flag button knows what to report.
    lastTranslation = {
      from: srcLang.value,
      to: tgtLang.value,
      original: text,
...
// Report a wrong/bad translation.
flagBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    showMessage("Thanks - noted for review.");
  } catch (err) {
    showMessage("Couldn't send that - please try again.");
  } finally {
...
photoInput.addEventListener("change", async () => {
  ...
  try {
    ...
    if (!text) {
      inputText.value = "";
      showMessage("Couldn't find any readable text in that photo.");
    } else {
      inputText.value = text;
    }
  } catch (err) {
    inputText.value = "";
    showMessage("Couldn't read that photo - please try again.");
  } finally {
