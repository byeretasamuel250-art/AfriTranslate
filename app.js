...
translateBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (err.message === "SUBSCRIPTION_REQUIRED") {
      showPaywall();
    } else if (err.message !== "SESSION_CONFLICT") {
...
async function translateOneChunk(text, from, to) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 402 || data.subscription_required) {
      // The server itself just refused this because there's no active
      // subscription (see _lib/subscription.js) - this can happen even
      // if the paywall was hidden a moment ago, e.g. a subscription that
      // just expired. Show the real paywall rather than a generic error.
