...
async function startSubscriptionPayment(errorEl) {
  ...
  if (isSessionConflict(response, data)) return false;
...
// duration defaults to a length-aware value: short messages ("Type
// something first") still clear quickly, but a longer error message -
// like a real backend error someone actually needs to read - stays up
// long enough to read instead of vanishing after a fixed 1.8s
// regardless of how much text there is.
function showMessage(text, duration) {
  if (duration === undefined) {
    duration = Math.min(Math.max(text.length * 60, 1800), 8000);
  }
...
translateBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    // SESSION_CONFLICT already signed the tab out and showed its own
    // message (see isSessionConflict/handleSessionConflict in auth.js) -
    // no need to also show a translation-specific error on top of that.
    if (err.message !== "SESSION_CONFLICT") {
      // Show the SPECIFIC reason the backend gave (e.g. "Sunbird isn't
      // responding right now...", "Text is too long...") rather than
      // hiding it behind one generic message - a generic message here
      // means neither we nor the person using the app can ever tell
      // what actually failed. RATE_LIMITED is a special sentinel (no
      // real message attached), so it still gets its own friendly text.
...
async function translateOneChunk(text, from, to) {
  ...
  if (isSessionConflict(response, data)) {
...
async function startSunbirdVoiceInput(languageCode) {
  ...
  mediaRecorder.onstop = async () => {
    ...
    try {
      ...
      inputText.value = "";
      showMessage(err.message || "Couldn't transcribe that - please try again.");
    }
  };
