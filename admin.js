...
function formatSubscriptionCell(user) {
  if (user.subscription_active) {
    return `<span class="badge active">Active until ${formatDate(user.subscription_period_end)}</span>`;
  }
  if (user.subscription_period_end) {
    // Has a subscriptions row, but current_period_end is in the past (or
    // status isn't "active") - same "expired" outcome the paywall itself
    // shows them, so the label here matches what they're actually seeing.
    return `<span class="badge banned">Expired ${formatDate(user.subscription_period_end)}</span>`;
  }
  return '<span class="muted">Never subscribed</span>';
}
...
function renderUsers(users) {
  ...
  for (const user of users) {
    ...
    row.innerHTML = `
      ...
      <td>${formatSubscriptionCell(user)}</td>
